...
class User_Settings extends CI_Controller {
    ...
    public function index() {
        ...
        $lead_types = $this->user_model->get_lead_types_by_user_id($this->user->info->ID);
    ...
    public function pro() {
        ...
        $phone_no = $this->common->nohtml($this->input->post("phone_no"));
        $aboutme = $this->common->nohtml($this->input->post("aboutme"));

		//selected lead types
			
		$selected_tags = $this->input->post("selected_tags", true);
			
		$lead_types  = array();
		
		if(isset($selected_tags))
			$lead_types = explode(',',$selected_tags);
		
		
		//Update selected lead types as well		
		$this->user_model->save_user_lead_types($this->user->info->ID,$lead_types);
