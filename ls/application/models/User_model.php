...
class User_Model extends CI_Model 
{
	...
	{		
		return $this->db	
                ->select("id,name,email,phone,token,aff_id,ip, DATE_FORMAT(FROM_UNIXTIME(timestamp),'%Y%c%d') as created_date,lead_types,notes")
				->get('questionnaire');
	...
	{		
		...
		$this->db->select("id,name,email,phone,token,aff_id,purchased_date,phone_no,lead_types");
		$this->db->from("v_lead_purchases");
		$this->db->where("phone_no", $phone_no);
		$rs = $this->db->get();	
		$lead_purchases = $rs->result_array();
		$lead_purchases_arr = array();
		if(count($lead_purchases) > 0){
			foreach($lead_purchases as $lead){
				$this->db->select("first_name,last_name");
				$this->db->from("customer_purchases");
				$this->db->join("users", "users.ID = customer_purchases.stripeEmail_email_username");
				$this->db->where("customer_purchases.id", $lead['id']);
				$rs = $this->db->get();	
				$user_arr = $rs->result_array();
				$accepters = '';
				if(count($user_arr) > 0){
					$user_name = array();
					...
					$accepters = implode(',', $user_name);
				}
				$lead['accepters'] = $accepters;
				$lead_purchases_arr[] =$lead; 
			}
			
		} 	
		return $lead_purchases_arr;		
