...
class User_Model extends CI_Model 
{
	...
	{		
		return $this->db	
				->select("id,name,email,phone,token,aff_id,ip, DATE_FORMAT(FROM_UNIXTIME(timestamp),'%Y%c%d') as created_date")
				->get('questionnaire');
	...
	{		
		return $this->db
			->where("phone_no", $phone_no)			
			->select("id,name,email,phone,token,aff_id,purchased_date,phone_no")
	...
	public function get_customer_purchase($id) 
	{
		$s= $this->db
			->where("id",$id)			
			->select("id,stripeEmail_email_username,stripeToken,token,purchased_timestamp,cost,phone_no")
			->get("customer_purchases");
	...
	{
		$s= $this->db			
			->select("id,header_color,button_color,button_hover_color,button_border_color,lead_sms_template,charge_sms_template,lead_cost")
			->get("custom_settings");
		return $s->row();
	...
	public function get_app_keys() 
	{
		$s= $this->db	
			->where("1=1")
			->select("id,stripe_secret_key,stripe_publishable_key,twilio_sid,twilio_auth_token")
			->get("app_keys");
		return $s->row();
			
	}
	public function save_app_keys($id,$stripe_secret_key,$stripe_publishable_key,$twilio_sid,$twilio_auth_token)
	{
		try{
			if(isset($id))
			{
	...
	public function save_customization($lead_sms_template,$charge_sms_template,$header_color,$button_color,$button_hover_color,$button_border_color,$lead_cost)
	{
		try{
			// if(isset($id))
			// {
