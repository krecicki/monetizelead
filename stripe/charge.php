...
<div class="row navbar-inverse" style="margin: -20px -20px 20px -20px; padding: 20px;">
	...
	<?php if(!is_null($charge)):
		$tokenidResults = $db->where('token',$tokenid)->orderBy('id','DESC')->getOne('questionnaire');
			$name = $tokenidResults['name'];
			$phone = $tokenidResults['phone'];
			$email = $tokenidResults['email'];
			$aff_id = $tokenidResults['aff_id'];
			$click_hash = $tokenidResults['click_hash'];
			$tokenid = $tokenidResults['token'];
			//$has_submitted = $row['has_submitted'];
			$units_sold = $tokenidResults['units_sold'];
	?>		
