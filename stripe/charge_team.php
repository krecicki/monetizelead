		...
		{		
			...
			try {
				...
				{						
					...
					//Wrong query param mysql_query("UPDATE questionnaire SET units_sold = units_sold+1 WHERE 1=1;");	
						
					$update_unit = array();
                                        $update_unit['units_sold'] = $units_sold + 1;
                                        $db->where ('token', $tokenid);
                                        $db->update ('questionnaire', $update_unit);
					...
					$db->insert('customer_purchases',Array(
						'stripeEmail_email_username'=>base64_decode($_POST['uid']), 'token'=>$tokenid, 'purchased_timestamp'=>time(), 'stripeToken'=>$_POST['tokent'], 'units_sold'=>$units_sold,'cost'=>($lead_cost/100),'phone_no'=>$phone_no
					));
...
<head>
    ...
    <title>Monetizelead::Lead Contact Details</title>
...
<div class="row navbar-inverse" style="margin: -20px -20px 20px -20px; padding: 20px;">
    ...
    <div class="row">
        <div class="col-xs-10 col-sm-10 col-md-6 col-xs-offset-1 col-sm-offset-1 col-md-offset-3">
           <?php  echo '<strong>Leads Unique ID:</strong> ', $tokenid;?>
        </div>
    </div>	
	<?php if(!is_null()):
	...
	<div class="row">
        <div class="col-xs-10 col-sm-10 col-md-6 col-xs-offset-1 col-sm-offset-1 col-md-offset-3">
			...
			<p>
				<strong>Tags: </strong> <?php echo $lt;?>
			</p>
			<!--
			<p>
