...
if(!empty($row))
{
	...
	$lead_rate_limit = $row['lead_rate_limit'];
}	
...
<head>
    ...
    <title>Monetizelead::Accept Lead</title>
        ...
        <div class="well col-xs-10 col-sm-10 col-md-6 col-xs-offset-1 col-sm-offset-1 col-md-offset-3">
            ...
            <div class="row">
                ...
                <div class="col-xs-6 col-sm-6 col-md-6 text-right">
					...
					<p>
						<strong>Accept Rate Limit:</strong>
						<br>
						<span class="text-muted"><?php echo $lead_rate_limit;?></span>
					</p>
                </div>
            </div>
            <div class="row">   
				<form action="charge_team.php" method="post">
					<input type="hidden" name="token" value="<?php echo $tokenid?>"/>
					<input type="hidden" name="ntoken" value="<?php echo $phone_no?>"/>
					<input type="hidden" name="uid" value="<?php echo $uid?>"/>
					<input type="hidden" name="tokent" value="<?php echo $tokent?>"/>
				    <!--<script src="https://checkout.stripe.com/checkout.js" class="stripe-button"
					  data-key="<?php echo $stripe['publishable_key']; ?>"
					  data-amount="<?php echo $btnCost ?>" data-description="Lead Checkout"></script>-->
					<?php
					if ($lead_rate_limit == $units_sold) {
  						echo '<center><strong>Someone else beat you by accepting this lead first! Better luck next time!</strong></center>';
							}
						else
							{
  						echo '<button id="btnaccept" type="submit" class="btn btn-success btn-lg btn-block btn-pay">
